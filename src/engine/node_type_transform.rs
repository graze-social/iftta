    ...
    async fn evaluate(&self, node: &Node, input: &Value) -> Result<Option<Value>> {
        ...
        if node.payload.is_array() {
            ...
            for (index, transformation) in transformations.iter().enumerate() {
                ...
                // Apply the transformation (use template evaluation for multi-field objects)
                let result = evaluate_template(true, transformation, &current_data)?;
            ...
            // Process single object transformation (use template evaluation)
            let transformed = evaluate_template(true, &node.payload, input)?;
